...
void game(int socket,char buffer[],int *ganados,int *perdidos)
{
    pthread_t thread_game;   //thread para la partida asi aliviano al proceso
    pthread_create( &thread_game, NULL, thread_fun, NULL );
...
void estrategia(tlista_s *jugadas,char mov[],char mov_ant[],int bien, int regular,int *mi_bien,int *mi_regular)
{
    pthread_t thread_estr;   //thread aparte para la estrategia ya que es la parte mas pesada del TP
    pthread_create( &thread_estr, NULL, thread_fun, NULL );
